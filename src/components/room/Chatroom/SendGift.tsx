...
import { SEND_GIFT } from '@/gql/chatroom'
...
const SendGift: React.FC<GiftPorps> = ({
  ...
  const [createGift] = useMutation<types.SEND_GIFT, any>(SEND_GIFT)
  ...
  const isEnough = user?.profile?.balance < selectedGift[0]?.value
  ...
  return (
    <div ref={clickRef}>
      ...
      >
        <div className="p-3 w-full h-full bg-white rounded-md border border-gray-300">
          ...
          <div className="grid grid-cols-5 grid-rows-2 grid-flow-row w-full h-40 hover:cursor-pointer">
            {giftList.map((item, idx) => {
              return (
                <div
                  onClick={() => setGift(item.name)}
                  key={`gift-${idx}`}
                  className="p-2 m-auto rounded-md hover:bg-rose-400"
                >
          ...
          <div className="flex justify-between items-center w-full h-12">
            ...
            <div
              ...
              className="py-2 px-4 font-medium bg-amber-400 rounded-lg hover:bg-orange-500 hover:cursor-pointer active:bg-yellow-300 hover:text-theme-300 active:text-theme-50"
            >
      ...
      <div className="px-1">
        <div
          onClick={onClick}
          className="text-2xl text-theme-50/40 i-heroicons-gift-solid"
        ></div>
      </div>
    </div>
  )
